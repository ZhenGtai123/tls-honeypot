...
def build_report(
    ...
    for req in requests:
        ...
        if primary and cls not in ("reconnaissance", "unknown"):
            cve_id = primary["cve_id"]
            cve_counts[cve_id] += 1
            cve_paths[cve_id][path] += 1
            cve_meta[cve_id] = {
                "name": primary["name"],
        ...
        "cve_summary": {
            cve_id: {
                **cve_meta[cve_id],
                ...
                "top_paths": [
                    {"path": p, "count": c}
                    for p, c in cve_paths[cve_id].most_common(5)
                ],
            }
            for cve_id, count in cve_counts.most_common()
        },
        "attack_samples": {
...
def _format_cve_section(cve_summary: dict) -> List[str]:
    lines = []
    if not cve_summary:
        return lines
    lines.append("LIKELY CVE / EXPLOIT SIGNATURES")
    lines.append("-" * 40)
    lines.append("  Heuristic mapping from request paths, queries, and bodies.")
    lines.append("")
    for cve_id, info in cve_summary.items():
        paths = ", ".join(p["path"][:50] for p in info.get("top_paths", [])[:2])
        lines.append(
            f"  {info['count']:5,}  {cve_id:18s}  [{info['severity']:8s}]  "
            f"{info['product']}"
        )
        lines.append(f"         {info['name']} ({info['confidence']})")
        if paths:
...
def format_text_report(report: dict) -> str:
    ...
    for cls, samples in report["attack_samples"].items():
        ...
        for sample in samples[:5]:
            ...
            cve = (sample.get("primary_cve") or {}).get("cve_id", "")
            cve_tag = f"  {cve}" if cve else ""
            lines.append(
                f"  {sample['timestamp'][:19]}  {sample['client_ip']:18s}  "
                f"{sample['method']:6s}  {status}  {sample['path'][:45]}{cve_tag}"
            )
        lines.append("")
...
def build_classification_detail(
    ...
    for req in matched:
        ...
        cve_id = primary["cve_id"]
        cve_counts[cve_id] += 1
        cve_paths[cve_id][req.get("path", "")] += 1
        cve_meta[cve_id] = {
            "name": primary["name"],
            "product": primary["product"],
            "severity": primary["severity"],
...
def format_detail_report(detail: dict) -> str:
    ...
    lines.extend(_format_cve_section(detail.get("cve_summary") or {}))
    ...
    for i, event in enumerate(detail["events"], 1):
        ...
        if event.get("primary_cve"):
            ...
            lines.append(
                f"  Likely CVE:     {pc['cve_id']} — {pc['name']} "
                f"({pc['product']}, {pc['confidence']})"
            )
        elif event.get("cves"):
            lines.append(f"  Likely CVEs:    {', '.join(c['cve_id'] for c in event['cves'])}")
...
def build_cve_detail(requests: List[dict], traffic: List[dict], cve_id: str) -> dict:
    traffic_by_id = {t["request"]["request_id"]: t for t in traffic}
    cve_info = get_cve_info(cve_id)
    matched = [r for r in requests if request_matches_cve(r, cve_id)]
    ...
    for req in sorted(matched, key=lambda r: r["timestamp"]):
        ...
        event = {
            ...
            "cves": cves,
            "primary_cve": primary,
            "is_primary_match": bool(primary and primary["cve_id"].upper() == cve_id.upper()),
            "headers": req.get("headers", {}),
            "body": req.get("body"),
            "body_encoding": req.get("body_encoding"),
            "tls": req.get("tls", {}),
    ...
    return {
        "cve": cve_info or {"cve_id": cve_id},
        "summary": {
            "total_events": len(matched),
            "primary_matches": sum(1 for e in events if e["is_primary_match"]),
            "secondary_matches": sum(1 for e in events if not e["is_primary_match"]),
            "with_response": sum(1 for e in events if e["has_response"]),
            "without_response": sum(1 for e in events if not e["has_response"]),
            "unique_client_ips": len(client_ips),
            "unique_paths": len(paths),
...
def format_cve_detail_report(detail: dict) -> str:
    lines = []
    cve = detail["cve"]
    s = detail["summary"]
    tr = s["time_range"]
    ...
    lines.append("=" * 72)
    lines.append(f"CVE REPORT: {cve.get('cve_id', 'UNKNOWN')}")
    lines.append("=" * 72)
    lines.append("")
    if cve.get("name"):
        lines.append(f"Name:            {cve['name']}")
    if cve.get("product"):
        lines.append(f"Product:         {cve['product']}")
    if cve.get("severity"):
        lines.append(f"CVE severity:    {cve['severity']}")
    if cve.get("confidence"):
        lines.append(f"Match confidence: {cve['confidence']}")
    if cve.get("notes"):
        lines.append(f"Notes:           {cve['notes']}")
    lines.append("")
    lines.append("SUMMARY")
    lines.append("-" * 40)
    lines.append(f"  Matching requests:   {s['total_events']:,}")
    lines.append(f"  Primary signature:   {s['primary_matches']:,}")
    lines.append(f"  Secondary signature: {s['secondary_matches']:,}")
    lines.append(f"  With response:       {s['with_response']:,}")
    lines.append(f"  Without response:    {s['without_response']:,}")
    lines.append(f"  Unique attacker IPs: {s['unique_client_ips']:,}")
    lines.append(f"  Unique paths:        {s['unique_paths']:,}")
    ...
    if s.get("classifications"):
        lines.append("ATTACK CLASSIFICATIONS (for matching requests)")
        lines.append("-" * 40)
        for cls, info in s["classifications"].items():
            lines.append(
                f"  [{info['severity']:6s}] {info['count']:5,}  {info['label']}"
            )
        lines.append("")
    ...
    for i, event in enumerate(detail["events"], 1):
        ...
        lines.append(f"  Forwarded to:   {event.get('forwarded_to', '')}")
        lines.append(f"  Experiment:     {event.get('experiment_group', '')}")
        match_role = "primary" if event.get("is_primary_match") else "secondary"
        lines.append(f"  CVE match:      {match_role}")
        if event.get("cves"):
            other = [c["cve_id"] for c in event["cves"] if c["cve_id"].upper() != cve.get("cve_id", "").upper()]
            if other:
                lines.append(f"  Also matched:   {', '.join(other)}")
...
def list_cve_catalog() -> str:
    lines = ["CVE / exploit signature catalog:", ""]
    for entry in get_cve_catalog():
        lines.append(
            f"  {entry['cve_id']:22s}  [{entry['severity']:8s}]  "
            f"{entry['product']}"
        )
        lines.append(f"    {entry['name']}")
    lines.append("")
    lines.append("Mappings are heuristic — a match means the request resembles a known exploit.")
    lines.append("")
    lines.append("Usage: python3 parse_attacks.py --cve <cve_id>")
    lines.append("  e.g. python3 parse_attacks.py --all --cve CVE-2017-9841")
    return "\n".join(lines)
...
def default_cve_detail_paths(
    output_dir: Path, cve_id: str, requests_path: Optional[Path], aggregated: bool
) -> Tuple[Path, Path]:
    safe_id = cve_id.replace("/", "-")
    if aggregated:
        return (
            output_dir / f"cve-{safe_id}-all.txt",
            output_dir / f"cve-{safe_id}-all.json",
        )
    date = extract_log_date(requests_path) if requests_path else None
    suffix = f"-{date}" if date else ""
    return (
        output_dir / f"cve-{safe_id}{suffix}.txt",
        output_dir / f"cve-{safe_id}{suffix}.json",
    )
...
def main():
    ...
    parser.add_argument(
        "--cve",
        metavar="CVE_ID",
        help="Show full per-event breakdown for a CVE or technique (e.g. CVE-2017-9841)",
    )
    parser.add_argument(
        "--all",
        action="store_true",
    ...
    parser.add_argument(
        "--list-cves",
        action="store_true",
        help="List CVE / exploit signature rules and exit",
    )
    ...
    if args.detail and args.cve:
        print("Error: --detail and --cve cannot be used together.", file=sys.stderr)
        sys.exit(1)

    if args.cve:
        cve_id = resolve_cve_id(args.cve)
        if not cve_id:
            print(f"Error: unknown CVE or technique '{args.cve}'.\n", file=sys.stderr)
            print(list_cve_catalog(), file=sys.stderr)
            sys.exit(1)

        detail = build_cve_detail(requests, traffic, cve_id)
        text = format_cve_detail_report(detail)

        default_text, default_json = default_cve_detail_paths(args.output_dir, cve_id, req_path, aggregated)
